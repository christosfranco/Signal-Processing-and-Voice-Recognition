...
def which_set(filename, validation_percentage, testing_percentage):
  ...
  if percentage_hash < validation_percentage:
    ...
    result = 'training'
  return result
print(which_set('/commands/sheila/004ae714_nohash_0.wav',10,10))

file1 = open("train_list.txt","w") 
file2 = open("testing_list.txt","w") 
file3 = open("validation_list.txt","w") 
for dirpath, dirnames, filenames in os.walk('./commands'):
    #print(dirpath)
    for filename in filenames:
        path = str(dirpath)+'/'+str(filename)
        #print(path)
        filename = path[11:]
        print(filename)
        if which_set(path,10,10) == 'training':
            file1.write(filename+'\n')
        elif which_set(path,10,10) == 'validation':
            file3.write(filename+'\n')
        else:
